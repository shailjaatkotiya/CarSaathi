"""Amazon Location Service adapter for ride navigation."""

from __future__ import annotations

import json
from typing import Any
from urllib.error import HTTPError, URLError
from urllib.parse import quote
from urllib.request import Request, urlopen

from app.core.config import Settings
from app.core.exceptions import (
    ConfigurationError,
    ExternalServiceError,
    ValidationError,
)


class AmazonLocationService:
    def __init__(self, settings: Settings) -> None:
        self.settings = settings

    def calculate_route(
        self, origin_query: str, destination_query: str
    ) -> dict[str, Any]:
        api_key = self.settings.amazon_location_api_key.strip()
        if not api_key:
            raise ConfigurationError(
                "Amazon Location Service API key is not configured"
            )

        origin = self._geocode(origin_query, api_key)
        destination = self._geocode(destination_query, api_key)
        route = self._route(origin["position"], destination["position"], api_key)

        return {
            "provider": "amazon-location",
            "origin": {
                "label": origin["label"],
                "query": origin_query,
                "position": origin["position"],
            },
            "destination": {
                "label": destination["label"],
                "query": destination_query,
                "position": destination["position"],
            },
            **route,
        }

    def search(self, query: str, limit: int = 5) -> list[dict[str, Any]]:
        api_key = self.settings.amazon_location_api_key.strip()
        if not api_key:
            raise ConfigurationError(
                "Amazon Location Service API key is not configured"
            )
        text = query.strip()
        if not text:
            return []
        payload = {
            "QueryText": text,
            "MaxResults": max(1, min(limit, 10)),
            "Language": self.settings.amazon_location_language,
            "Filter": {"IncludeCountries": ["IND"]},
        }
        data = self._post("places", "/v2/autocomplete", payload, api_key)
        results: list[dict[str, Any]] = []
        for item in data.get("ResultItems") or []:
            label = (item.get("Title") or "").strip()
            if label:
                results.append({"label": label})
        return results

    def route(self, origin_query: str, destination_query: str) -> dict[str, Any]:
        return self.calculate_route(origin_query, destination_query)

    def route_alternatives(
        self,
        origin_query: str,
        destination_query: str,
        max_alternatives: int = 2,
        origin_position: list[float] | None = None,
        destination_position: list[float] | None = None,
    ) -> dict[str, Any]:
        api_key = self.settings.amazon_location_api_key.strip()
        if not api_key:
            raise ConfigurationError(
                "Amazon Location Service API key is not configured"
            )
        origin = (
            {"label": origin_query, "position": origin_position}
            if origin_position and len(origin_position) >= 2
            else self._geocode(origin_query, api_key)
        )
        destination = (
            {"label": destination_query, "position": destination_position}
            if destination_position and len(destination_position) >= 2
            else self._geocode(destination_query, api_key)
        )
        raw = self._fetch_routes(
            origin["position"],
            destination["position"],
            api_key,
            alternatives=max(0, min(max_alternatives, 5)),
        )
        options = [self._summarize_route(route_obj) for route_obj in raw]
        return {
            "provider": "amazon-location",
            "origin": {
                "label": origin["label"],
                "query": origin_query,
                "position": origin["position"],
            },
            "destination": {
                "label": destination["label"],
                "query": destination_query,
                "position": destination["position"],
            },
            "options": options,
        }

    def geocode(self, query: str) -> dict[str, Any]:
        api_key = self.settings.amazon_location_api_key.strip()
        if not api_key:
            raise ConfigurationError(
                "Amazon Location Service API key is not configured"
            )
        result = self._geocode(query.strip(), api_key)
        return {"label": result["label"], "position": result["position"]}

    def reverse_geocode(self, longitude: float, latitude: float) -> dict[str, Any]:
        api_key = self.settings.amazon_location_api_key.strip()
        if not api_key:
            raise ConfigurationError(
                "Amazon Location Service API key is not configured"
            )
        payload = {
            "QueryPosition": [longitude, latitude],
            "MaxResults": 1,
            "Language": self.settings.amazon_location_language,
        }
        data = self._post("places", "/v2/reverse-geocode", payload, api_key)
        result = (data.get("ResultItems") or [None])[0]
        if not result:
            raise ValidationError("Could not resolve an address for this location")
        return {
            "label": result.get("Title") or "Selected location",
            "position": result.get("Position") or [longitude, latitude],
        }

    def map_config(self) -> dict[str, Any]:
        api_key = self.settings.amazon_location_api_key.strip()
        if not api_key:
            raise ConfigurationError(
                "Amazon Location Service API key is not configured"
            )
        region = self.settings.amazon_location_region
        style = self.settings.amazon_location_map_style
        style_url = (
            f"https://maps.geo.{region}.amazonaws.com/v2/styles/{style}/descriptor"
            f"?key={quote(api_key)}"
        )
        return {
            "provider": "amazon-location",
            "region": region,
            "style": style,
            "api_key": api_key,
            "style_url": style_url,
        }

    def _geocode(self, query: str, api_key: str) -> dict[str, Any]:
        payload = {
            "QueryText": query,
            "MaxResults": 1,
            "Language": self.settings.amazon_location_language,
            "Filter": {"IncludeCountries": ["IND"]},
        }
        data = self._post("places", "/v2/geocode", payload, api_key)
        result = (data.get("ResultItems") or [None])[0]
        if not result or not result.get("Position"):
            raise ValidationError(f"Could not find map coordinates for {query}")
        return {
            "label": result.get("Title") or query,
            "position": result["Position"],
        }

    def _route(
        self, origin: list[float], destination: list[float], api_key: str
    ) -> dict[str, Any]:
        routes = self._fetch_routes(origin, destination, api_key, alternatives=0)
        return self._summarize_route(routes[0])

    def _fetch_routes(
        self,
        origin: list[float],
        destination: list[float],
        api_key: str,
        alternatives: int = 0,
    ) -> list[dict[str, Any]]:
        payload: dict[str, Any] = {
            "Origin": origin,
            "Destination": destination,
            "TravelMode": "Car",
            "TravelStepType": "Default",
            "InstructionsMeasurementSystem": "Metric",
            "Languages": [self.settings.amazon_location_language],
            "LegAdditionalFeatures": ["Summary", "TravelStepInstructions", "Tolls"],
            "LegGeometryFormat": "Simple",
            "Traffic": {"Usage": "UseTrafficData"},
        }
        if alternatives > 0:
            payload["MaxAlternatives"] = alternatives
        data = self._post("routes", "/v2/routes", payload, api_key)
        routes = data.get("Routes") or []
        if not routes:
            raise ValidationError("Amazon Location could not calculate a route")
        return routes

    @staticmethod
    def _summarize_route(route_obj: dict[str, Any]) -> dict[str, Any]:
        legs = route_obj.get("Legs") or []
        distance = 0.0
        duration = 0.0
        has_tolls = False
        steps: list[dict[str, Any]] = []
        geometry: list[list[float]] = []
        labels: list[str] = []
        for leg in legs:
            details = (
                leg.get("VehicleLegDetails") or leg.get("PedestrianLegDetails") or {}
            )
            summary = details.get("Summary") or {}
            overview = summary.get("Overview") or {}
            distance += float(overview.get("Distance") or 0)
            duration += float(overview.get("Duration") or 0)
            geometry.extend((leg.get("Geometry") or {}).get("LineString") or [])
            if details.get("Tolls"):
                has_tolls = True
            for step in details.get("TravelSteps") or []:
                instruction = (step.get("Instruction") or "").strip()
                if instruction:
                    steps.append(
                        {
                            "instruction": instruction,
                            "distance_meters": float(step.get("Distance") or 0),
                            "duration_seconds": float(step.get("Duration") or 0),
                        }
                    )
        for road in route_obj.get("MajorRoadLabels") or []:
            value = ((road.get("RouteNumber") or {}).get("Value")) or (
                (road.get("RoadName") or {}).get("Value")
            )
            if value and value not in labels:
                labels.append(value)
        return {
            "distance_meters": distance,
            "duration_seconds": duration,
            "steps": steps[:12],
            "geometry": geometry,
            "has_tolls": has_tolls,
            "road_label": ", ".join(labels[:2]),
        }

    def _post(
        self, service: str, path: str, payload: dict[str, Any], api_key: str
    ) -> dict[str, Any]:
        host = f"{service}.geo.{self.settings.amazon_location_region}.api.aws"
        url = f"https://{host}{path}?key={quote(api_key)}"
        body = json.dumps(payload).encode("utf-8")
        request = Request(
            url,
            data=body,
            headers={"Content-Type": "application/json"},
            method="POST",
        )
        try:
            with urlopen(request, timeout=12) as response:
                return json.loads(response.read().decode("utf-8"))
        except HTTPError as exc:
            message = exc.read().decode("utf-8", errors="ignore")
            raise ExternalServiceError(
                f"Amazon Location request failed with HTTP {exc.code}: {message or exc.reason}"
            ) from exc
        except (URLError, TimeoutError, json.JSONDecodeError) as exc:
            raise ExternalServiceError("Amazon Location request failed") from exc
